...
class Preprocessor:
    def __init__(self, input_folder, output_folder, preprocess=False, use_original_x=False):
        """
        :param preprocess: If True, process images with CLAHE & morphological operations
        :param use_original_x: If True, return X before CLAHE & morphological operations
        """
        self.input_folder = input_folder
        self.output_folder = output_folder
        self.preprocess = preprocess
        self.use_original_x = use_original_x  # Whether to return original X before processing
    ...
    def process_dicom_file(self, filepath):
        ...
        if self.preprocess:
            ...
            processed_x = clahe.apply(processed_x.astype(np.uint8)).astype(np.float32) / 255.0
        else:
            processed_x = original_x.astype(np.float32) / 255.0  # Normalize only
    ...
    def load_patient_data(self):
        ...
        return np.array(original_x_data), np.array(processed_x_data), np.array(y_data)
    ...
    def run(self):
        ...
        X_data = original_x if self.use_original_x else processed_x
